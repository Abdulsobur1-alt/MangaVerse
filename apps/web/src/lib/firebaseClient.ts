...
// Sign-in/sign-up for email+password using Firebase's public REST endpoints.
// Uses the web API key only (safe to expose client-side). Keeps both web and
// mobile on the same lightweight flow without adding the Firebase JS SDK.
...
async function firebaseRequest<T>(path: string, body: Record<string, unknown>): Promise<T> {
  if (!API_KEY) {
    throw new Error('Firebase is not configured (NEXT_PUBLIC_FIREBASE_API_KEY missing)');
  }
